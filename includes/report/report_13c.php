...
Class Report13C extends Report {
    ...
    public function setResult() {
         ...
         * 
         */
        //sql statement for 13c(Latest Follow Up Evaluation)
        if ($this->eval_method != 'both') {
            $sql1 = "CREATE OR REPLACE VIEW result_table_13c AS
            ...
            $sql2 = "create or replace view initial_eval_13c as
                 select evaluation.* 
                 from result_table_13c, evaluation 
                 where evaluation.eval_type = 1 and evaluation.farm_id = result_table_13c.farm_id and evaluation.eval_method = '" . "$this->eval_method" . "'
                 group by farm_id;";
        } else {
            $sql1 = "CREATE OR REPLACE VIEW result_table_13c AS
                 Select * from evaluation
            ...
            $sql2 = "create or replace view initial_eval_13c as
                 select evaluation.* 
                 from result_table_13c, evaluation 
                 where evaluation.eval_type = 1 and evaluation.farm_id = result_table_13c.farm_id
                 group by farm_id;";
        }
        $sql_array = array($sql1, $sql2);
        MIL::createView($sql_array);
        $sql22 = "select distinct(initial_eval_13c.irr_sys_type), irr_sys_types.common_name 
        		  from initial_eval_13c, irr_sys_types 
	              where initial_eval_13c.irr_sys_type = irr_sys_types.id 
                  order by irr_sys_types.common_name";

        $groupByirrType = MIL::doQuery($sql22, MYSQL_ASSOC);
        //$groupByCrop = array(array("crop_category" => 9));
        foreach ($groupByirrType as $key => $array) {
            $sql33 = "CREATE OR REPLACE VIEW initial_eval_13c_by_irr AS
                        SELECT * FROM initial_eval_13c
        		  WHERE irr_sys_type = $array[irr_sys_type];";
            $sql_array = array($sql33);
            MIL::createView($sql_array);
            $sql3 = "create or replace view start_date_13c as
                    ...
                    from initial_eval_13c_by_irr
                    group by county_id;";

            $sql4 = "create or replace view farm_max_end_date_13c as 
                    select result_table_13c.farm_id, max(result_table_13c.eval_yr_month) end_date
                    from result_table_13c, initial_eval_13c_by_irr
                    where result_table_13c.farm_id = initial_eval_13c_by_irr.farm_id
                    group by farm_id;";

            $sql5 = "create or replace view latest_follow_up_eval_13c as
                select county_id, sum(acre) Latest_Follow_Up_Evaluation, format(sum(aws), 2) aws, date_format(max(eval_yr_month), '%Y-%m') end_date
                from result_table_13c, farm_max_end_date_13c
                where result_table_13c.eval_yr_month = farm_max_end_date_13c.end_date and result_table_13c.farm_id = farm_max_end_date_13c.farm_id
                group by county_id";
            ...
            if ($this->eval_method != 'both') {
                $sql0 = "create or replace view first_followup_ordered_view as
                    ...
                    from evaluation, initial_eval_13c_by_irr
                    where evaluation.eval_type = 2 and evaluation.farm_id = initial_eval_13c_by_irr.farm_id and evaluation.eval_method = '" . "$this->eval_method" . "'
                    group by farm_id";   //farm id with more than one follow up evaluation

                $sql7 = "CREATE OR REPLACE VIEW first_followup_result_table_13c as
                ...
                $sql0 = "create or replace view first_followup_ordered_view as
                    select evaluation.farm_id, min(evaluation.eval_yr_month) eval_yr_month
                    from evaluation, initial_eval_13c_by_irr
                    where evaluation.eval_type = 2 and evaluation.farm_id = initial_eval_13c_by_irr.farm_id
                    group by farm_id";   //farm id with more than one follow up evaluation
            ...
            $sql8 = "select start_date_13c.county_id, num_of_irr_sys, Initial_Evaluation, First_Follow_Up_Evaluation,
                    Latest_Follow_Up_Evaluation, pws, latest_follow_up_eval_13c.aws as latest_aws,
                    first_followup_result_table_13c.aws as first_aws,
                    start_date, latest_follow_up_eval_13c.end_date as latest_end_date, 
                    first_followup_result_table_13c.end_date as first_end_date
                    from ((start_date_13c left join latest_follow_up_eval_13c on start_date_13c.county_id = latest_follow_up_eval_13c.county_id) left join first_followup_result_table_13c
                    on start_date_13c.county_id = first_followup_result_table_13c.county_id);";

            $sql_array_13c = array($sql0, $sql1, $sql2, $sql3, $sql4, $sql5, $sql7); // $sql6;
            //$this->results['farm_ids'] = $farm_ids;
            MIL::createView($sql_array_13c);
            //MIL::createView($sql_array_13c);
        ...
        $drop_view_sql = "drop view if exists result_table_13c,initial_eval_13c,initial_eval_13c_by_irr,start_date_13c,"
                . "farm_max_end_date_13c,latest_follow_up_eval_13c,first_followup_result_table_13c,first_followup_ordered_view";
...
<thead><tr>
  ...
  <th>Total AWS (% of Total PWS)</th>
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 13c: <br />
            Ag MIL Program Potential vs Actual Water Saved - by Irrigation System and County<br />
    ...
    public function getTableContent() {
        ...
        $this->tableContent = "";
        foreach ($this->results as $crop => $content) {
            ...
            $total = array("num_of_irr_sys" => 0,
                ...
                "latest_end_date" => 'N/A',
                "first_end_date" => 'N/A');
            $this->tableContent .= "Irrigation System at Initial Evaluation: {$name}<br/><br/>";
        ...
        $this->tableContent .=
            ...
            Total AWS: Actual Water Saving, from follow up evaluation, which happened during the oldest follow up evaluation data range only; and first follow up evaluations which happened during and/or prior to the latest follow up evaluation date range.<br />
