...
Class Report12A extends Report {
    ...
    public function setResult() {
        ...
        $sql1 = "CREATE OR REPLACE VIEW first_followup_12a AS  
        ...
        //find farm_id with only one first_followup_evaluation


        $sql2 = "CREATE OR REPLACE VIEW result_table_12a AS
                SELECT evaluation.* FROM evaluation INNER JOIN first_followup_12a
                ON evaluation.farm_id = first_followup_12a.farm_id
                WHERE (mil_id in $labs_str and evaluation.eval_type=1) or evaluation.id=first_followup_12a.id;";
        ...
        //if($method == 'both'){ 
        $sql3 = "select distinct(result_table_12a.crop_category) from result_table_12a, ag_urban_types_names 
		          where eval_type = 1 and result_table_12a.crop_category = ag_urban_types_names.id 
		           order by ag_urban_types_names.name";
        //}
        // else{
//         	$sql3 = "select distinct(result_table_12a.crop_category) from result_table_12a, ag_urban_types_names 
// 		          where eval_type = 1 and result_table_12a.crop_category = ag_urban_types_names.id 
// 		          and result_table_12a.eval_method = '"."{$method}"."'
//                             order by ag_urban_types_names.name";
//         }
        $groupByCrop = MIL::doQuery($sql3, MYSQL_ASSOC);
        //var_dump($groupByCrop);
        foreach ($groupByCrop as $key => $array) {

            // find initial evaluation filtered with crop category
            $sql4 = "CREATE OR REPLACE VIEW initial_eval_table_12a AS "
                    . "SELECT * from result_table_12a "
                    . "where crop_category = $array[crop_category] and eval_type = 1;";
            // find first follow up evaluation with crop category
            $sql5 = "CREATE OR REPLACE VIEW first_follow_up_table_12a AS "
                    . "SELECT result_table_12a.* "
                    . "from result_table_12a, initial_eval_table_12a "
                    . "where result_table_12a.eval_type = 2 and result_table_12a.farm_id = initial_eval_table_12a.farm_id";
//                        // start date table
...
//                                . "group by county_id";
            ...
            $sql6 = "CREATE OR REPLACE VIEW initial_eval_sum_12a AS "
                    ...
                    . "from initial_eval_table_12a "
                    . "group by county_id";
            // first_follow_up_eval_sum_12a
            $sql7 = "CREATE OR REPLACE VIEW first_follow_up_eval_sum_12a AS "
                    . "SELECT county_id, sum(acre) Follow_Up_Evaluation, format(sum(aws), 2) aws, date_format(max(eval_yr_month), '%Y-%m') end_date "
                    . "from first_follow_up_table_12a "
                    . "group by county_id";

            $sql8 = "select initial_eval_sum_12a.county_id, num_of_irr_sys, "
                    . "Initial_Evaluation, Follow_Up_Evaluation, "
                    . "initial_eval_sum_12a.pws, first_follow_up_eval_sum_12a.aws, start_date, end_date "
                    . "from initial_eval_sum_12a "
                    . "left join first_follow_up_eval_sum_12a on initial_eval_sum_12a.county_id = first_follow_up_eval_sum_12a.county_id";
        ...
        $drop_view_sql = "drop view if exists first_followup_ordered_view, first_followup_12a,result_table_12a, initial_eval_table_12a,"
                . "first_follow_up_eval_sum_12a,first_follow_up_table_12a,initial_eval_sum_12a";
...
<tr>
  ...
  <th width="9%">First Follow Up Evaluation Latest Date</th>
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 12A: <br />
            Ag MIL Program Potential vs Actual Water Savings, by Crop at Initial Evaluation and County<br />
