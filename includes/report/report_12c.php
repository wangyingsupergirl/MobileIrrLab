...
Class Report12C extends Report {
    ...
    public function init($array) {
        ...
        $sql = "select id, name from ag_urban_types_names";
    ...
    public function setResult() {
         ...
         * 
         */
        //sql statement for 12c(Latest Follow Up Evaluation)
        if ($this->eval_method != 'both'){
        $sql1 = "CREATE OR REPLACE VIEW result_table_12c AS
        ...
        $sql2 = "create or replace view initial_eval_12c as
                 select evaluation.* 
                 from result_table_12c, evaluation 
                 where evaluation.eval_type = 1 and evaluation.farm_id = result_table_12c.farm_id and evaluation.eval_method = '"."{$this->eval_method}"."'"
                 ."group by farm_id;";
		}
		else{
		        $sql1 = "CREATE OR REPLACE VIEW result_table_12c AS
        ...
        $sql2 = "create or replace view initial_eval_12c as
                 select evaluation.* 
                 from result_table_12c, evaluation 
                 where evaluation.eval_type = 1 and evaluation.farm_id = result_table_12c.farm_id
                 group by farm_id;";
		}
        $sql_array = array($sql1, $sql2);
        MIL::createView($sql_array);
        $sql22 = "select distinct(initial_eval_12c.crop_category) from initial_eval_12c, ag_urban_types_names 
	          where initial_eval_12c.crop_category = ag_urban_types_names.id 
                  order by ag_urban_types_names.name";
        
        $groupByCrop = MIL::doQuery($sql22, MYSQL_ASSOC);
        //$groupByCrop = array(array("crop_category" => 9));
        foreach ($groupByCrop as $key => $array) {
            $sql33 = "CREATE OR REPLACE VIEW initial_eval_12c_by_crop AS
                      SELECT * FROM initial_eval_12c
        		  WHERE crop_category = $array[crop_category];";
            $sql_array = array($sql33);
            MIL::createView($sql_array);
            $sql3 = "create or replace view start_date_12c as
                    ...
                    from initial_eval_12c_by_crop
                    group by county_id;";

            $sql4 = "create or replace view farm_max_end_date_12c as 
                    select result_table_12c.farm_id, max(result_table_12c.eval_yr_month) end_date
                    from result_table_12c, initial_eval_12c_by_crop
                    where result_table_12c.farm_id = initial_eval_12c_by_crop.farm_id
                     group by farm_id;";

            $sql5 = "create or replace view latest_follow_up_eval_12c as
                select county_id, sum(acre) Latest_Follow_Up_Evaluation, format(sum(aws), 2) aws, date_format(max(eval_yr_month), '%Y-%m') end_date
                from result_table_12c, farm_max_end_date_12c
                where result_table_12c.eval_yr_month = farm_max_end_date_12c.end_date and result_table_12c.farm_id = farm_max_end_date_12c.farm_id
                group by county_id";

            //sql statement for first follow up evaluation
            //first follow up may not be in the selected date range
            if ($this->eval_method != 'both'){
            $sql0 = "create or replace view first_followup_ordered_view as
                    ...
                    from evaluation, initial_eval_12c_by_crop
                    where evaluation.eval_type = 2 and evaluation.farm_id = initial_eval_12c_by_crop.farm_id and evaluation.eval_method = '"."$this->eval_method"."'
                    group by farm_id";   //farm id with more than one follow up evaluation

            $sql7 = "CREATE OR REPLACE VIEW first_followup_result_table_12c as
            ...
            $sql0 = "create or replace view first_followup_ordered_view as
                    select evaluation.farm_id, min(evaluation.eval_yr_month) eval_yr_month
                    from evaluation, initial_eval_12c_by_crop
                    where evaluation.eval_type = 2 and evaluation.farm_id = initial_eval_12c_by_crop.farm_id
                    group by farm_id";   //farm id with more than one follow up evaluation
            ...
            $sql8 = "select start_date_12c.county_id, num_of_irr_sys, Initial_Evaluation, First_Follow_Up_Evaluation,
                    Latest_Follow_Up_Evaluation, pws, latest_follow_up_eval_12c.aws as latest_aws,
                    first_followup_result_table_12c.aws as first_aws,
                    start_date, latest_follow_up_eval_12c.end_date as latest_end_date, 
                    first_followup_result_table_12c.end_date as first_end_date
                    from ((start_date_12c left join latest_follow_up_eval_12c on start_date_12c.county_id = latest_follow_up_eval_12c.county_id) left join first_followup_result_table_12c
                    on start_date_12c.county_id = first_followup_result_table_12c.county_id);";

            $sql_array_12c = array($sql0, $sql1, $sql2, $sql3, $sql4, $sql5, $sql7); // $sql6;

            //$this->results['farm_ids'] = $farm_ids;
            MIL::createView($sql_array_12c);
            //MIL::createView($sql_array_12c);
            ...
            if (is_array($query_results)) {
                ...
                $this->results[$array['crop_category']] = $temp;
            }
        }
        //$this->results=MIL::doQuery($sql, MYSQL_ASSOC);
        $drop_view_sql = "drop view if exists result_table_12c,initial_eval_12c,initial_eval_12c_by_crop,start_date_12c,"
                . "farm_max_end_date_12c,latest_follow_up_eval_12c,first_followup_result_table_12c,first_followup_ordered_view";
...
<thead><tr>
  ...
  <th>Total AWS (% of Total PWS)</th>
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 12C: <br />
            Ag MIL Program Potential vs Actual Water Saved - by Crop at Initial Evaluation and County<br />
    ...
    public function getTableContent() {
        ...
        foreach ($this->results as $crop => $content) {
            foreach ($this->cropTypes as $k => $names) {
                if ($names['id'] == $crop) {
                    $name = $names['name'];
                    break;
                }
            ...
            $total = array("num_of_irr_sys" => 0,
                ...
                "latest_end_date" => 'N/A',
                "first_end_date" => 'N/A');
            $this->tableContent .= "Crop at Initial Evaluation: {$name}<br/><br/>";
