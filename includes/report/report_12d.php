...
Class Report12D extends Report {
    ...
    public function setResult() {
        ...
        $this->eval_method = $this->params['eval_method'];   // changing
...
// 		print_r($method);
            $sql1 = "create or replace view replaced_table_12d as
                 select id, farm_id, county_id, acre, aws, pws, eval_yr_month
                 from evaluation
            ...
            $sql2 = "CREATE OR REPLACE VIEW init_crop_category_table_12d as
                SELECT distinct(evaluation.crop_category),replaced_table_12d.farm_id from evaluation, replaced_table_12d
                WHERE evaluation.eval_type =1 and evaluation.farm_id = replaced_table_12d.farm_id and evaluation.eval_method ='" . "{$this->eval_method}" . "'"
                    . "group by replaced_table_12d.farm_id";
        } else {
//      print_r($method);
            $sql1 = "create or replace view replaced_table_12d as
                 select id, farm_id, county_id, acre, aws, pws, eval_yr_month
            ...
            $sql2 = "CREATE OR REPLACE VIEW init_crop_category_table_12d as
                SELECT distinct(evaluation.crop_category),replaced_table_12d.farm_id from evaluation, replaced_table_12d
                WHERE evaluation.eval_type =1 and evaluation.farm_id = replaced_table_12d.farm_id
            	Group by replaced_table_12d.farm_id";
        }

        MIL::createView(array($sql1, $sql2));

        $sql3 = "select init_crop_category_table_12d.crop_category from init_crop_category_table_12d, ag_urban_types_names 
		         where init_crop_category_table_12d.crop_category = ag_urban_types_names.id 
				 order by ag_urban_types_names.name";
        $groupByCrop = MIL::doQuery($sql3, MYSQL_ASSOC);

        foreach ($groupByCrop as $key => $array) {

            $sql5 = "select replaced_table_12d.county_id, count(distinct(replaced_table_12d.farm_id)) num_of_irr_sys,sum(acre) total_acres, format(sum(aws), 2) aws,
                 date_format(min(eval_yr_month), '%Y-%m') start_date, date_format(max(eval_yr_month), '%Y-%m') end_date
     	       	from replaced_table_12d, init_crop_category_table_12d
     	       	where init_crop_category_table_12d.crop_category= $array[crop_category] and replaced_table_12d.farm_id = init_crop_category_table_12d.farm_id
                group by replaced_table_12d.county_id";
        ...
        //print_r($params);		
//         echo "</pre>";
        $drop_view_sql = "drop view if exists init_crop_category_table_12d, replaced_table_12d";
...
<tr>
  ...
  <th width="9%">Total Replaced Evaluations Acres</th>
  <th width="9%">Total AWS (Million Gallons/Day)</th>
  <th width="9%">Replaced Evaluation Earliest Date</th>
  <th width="9%">Replaced Evaluation Latest Date</th>
  </tr>';
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 12D: <br />
            Ag MIL Program Actual Water Savings, by Crop at Initial Evaluation and County <br /> 
