...
Class Report13A extends Report {
    ...
    public function setResult() {
        ...
        $sql1 = "CREATE OR REPLACE VIEW first_followup_13a AS  
        ...
        $sql2 = "CREATE OR REPLACE VIEW result_table_13a AS
                SELECT evaluation.* FROM evaluation INNER JOIN first_followup_13a
                ON evaluation.farm_id = first_followup_13a.farm_id
                WHERE (mil_id in $labs_str and evaluation.eval_type=1) or evaluation.id=first_followup_13a.id;";
        ...
        $sql3 = "select distinct(irr_sys_types.common_name)
		          from result_table_13a, irr_sys_types
		          where eval_type = 1 and result_table_13a.irr_sys_type = irr_sys_types.id 
...
// 		          from result_table_13a, irr_sys_types
// 		          where eval_type = 1 and result_table_13a.irr_sys_type = irr_sys_types.id 
// 		          and result_table_13a.eval_method = '"."{$method}"."'
// 				  order by irr_sys_types.common_name;";
// 			 } 
        ...
        foreach ($groupByIrri as $key => $array) {
            ...
            $sql4 = "CREATE OR REPLACE VIEW initial_eval_table_13a AS "
                    . "SELECT result_table_13a.*,irr_sys_types.common_name from result_table_13a,irr_sys_types "
                    . "where  eval_type = 1 and result_table_13a.irr_sys_type = irr_sys_types.id "
                    . " and irr_sys_types.common_name = '$array[common_name]' ";

            // find first follow up evaluation with crop category
            $sql5 = "CREATE OR REPLACE VIEW first_follow_up_table_13a AS "
                    . "SELECT result_table_13a.* "
                    . "from result_table_13a, initial_eval_table_13a "
                    . "where result_table_13a.eval_type = 2 and result_table_13a.farm_id = initial_eval_table_13a.farm_id";
//                        // start date table
...
//                                . "group by county_id";
            ...
            $sql6 = "CREATE OR REPLACE VIEW initial_eval_sum_13a AS "
                    ...
                    . "from initial_eval_table_13a "
                    . "group by county_id";
            // first_follow_up_eval_sum_13a
            $sql7 = "CREATE OR REPLACE VIEW first_follow_up_eval_sum_13a AS "
                    . "SELECT county_id, sum(acre) Follow_Up_Evaluation, format(sum(aws), 2) aws, date_format(max(eval_yr_month), '%Y-%m') end_date "
                    . "from first_follow_up_table_13a "
                    . "group by county_id";

            $sql8 = "select initial_eval_sum_13a.county_id, num_of_irr_sys, "
                    . "Initial_Evaluation, Follow_Up_Evaluation, "
                    . "initial_eval_sum_13a.pws, first_follow_up_eval_sum_13a.aws, start_date, end_date "
                    . "from initial_eval_sum_13a "
                    . "left join first_follow_up_eval_sum_13a on initial_eval_sum_13a.county_id = first_follow_up_eval_sum_13a.county_id";
            ...
            if (is_array($query_rez)) {
                ...
                $this->results[$array['common_name']] = $temp;
            }
        }
        //echo "<pre>";
        //print_r($this->results);
        //echo "</pre>";
        $drop_view_sql = "drop view if exists first_followup_ordered_view, first_followup_13a,result_table_13a, initial_eval_table_13a,"
                . "first_follow_up_eval_sum_13a,first_follow_up_table_13a,initial_eval_sum_13a";
...
<tr>
  ...
  <th width="9%">First Follow Up Evaluation Latest Date</th>
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 13A: <br />
            Ag MIL Program Potential vs Actual Water Savings, by Irrigation System Type and County<br />
    ...
    public function getTableContent() {
        ...
        foreach ($this->results as $irri => $content) {
            ...
            $total = array("num_of_irr_sys" => 0,
                ...
                "start_date" => 'N/A',
                "end_date" => 'N/A');
            $this->tableContent .= "Irrigation System Type: {$irri}<br/><br/>";
