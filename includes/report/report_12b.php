...
Class Report12B extends Report {
    ...
    public function setResult() {
        ...
        if ($this->eval_method == 'both') {
            ...
            $sql1 = "create or replace view initial_eval_12b as
                 	select * from evaluation
                 	where mil_id in $labs_str and farm_id in $farm_ids and status = 'approved' and evaluation.eval_type = 1  
			and eval_yr_month <= $end_date and eval_yr_month >= $start_date;";
        } else {
            $sql1 = "create or replace view initial_eval_12b as
                 	select * from evaluation
                 	where mil_id in $labs_str and farm_id in $farm_ids and status = 'approved' and evaluation.eval_type = 1 
        ...
        //if($method == 'both'){
        $sql2 = "select distinct(initial_eval_12b.crop_category) from initial_eval_12b, ag_urban_types_names 
		 	where eval_type = 1 and initial_eval_12b.crop_category = ag_urban_types_names.id 
		 	order by ag_urban_types_names.name";
        // }
        // else{
// 		 	$sql2 = "select distinct(initial_eval_12b.crop_category) from initial_eval_12b, ag_urban_types_names 
// 		 	where eval_type = 1 and initial_eval_12b.crop_category = ag_urban_types_names.id
// 		 	and initial_eval_12b.eval_method = '"."{$method}"."'
// 		 	order by ag_urban_types_names.name";
// 		 }
//         
        ...
        foreach ($groupByCrop as $key => $array) {
            ...
            $sql3 = "CREATE OR REPLACE VIEW initial_eval_12b_by_crop AS
                    SELECT * FROM initial_eval_12b
        ...
        $drop_view_sql = "drop view if exists initial_eval_12b,initial_eval_12b_by_crop";
...
<tr>
  ...
  <th width="9%">Initial Evaluation Latest Date</th>
    ...
    public function getReportTitle() {
        ...
        $this->reportTitle = " <h2 style='text-align:left;font-family:helvetica, arial, sans-serif;margin: 25px 10px 0 10px; padding-bottom: 0;'>Report 12B: <br />
            Ag MIL Program Potential Water Savings - by Crop at Initial Evaluation and County<br />
    ...
    public function getTableContent() {
        ...
        foreach ($this->results as $crop => $content) {
            ...
            $total = array("num_of_irr_sys" => 0,
                ...
                "start_date" => 'N/A',
                "end_date" => 'N/A');
            $this->tableContent .= "Crop: {$name}<br/><br/>";
